...
class GmailCreateDraftsFromResponsesInput(BaseModel):
    """Input schema for creating Gmail drafts from thread responses"""
    thread_responses: List[Dict[str, str]] = Field(..., description="List of dictionaries with 'thread_id' and 'response' keys")
 
class GmailCreateDraftsFromResponsesTool(BaseTool):
    name: str = "gmail_create_drafts_from_responses"
    description: str = "Creates Gmail draft replies from thread responses containing thread_id and response content"
    args_schema: Type[BaseModel] = GmailCreateDraftsFromResponsesInput
 
    def _run(self, thread_responses: List[Dict[str, str]]) -> str:
        """
